...
from from_goodfeli.model import sigmoid_kl_with_logits
from from_goodfeli.ops import variables_on_gpu0
from from_goodfeli.ops import avg_grads
...
def build_model_single_gpu(self, gpu_idx):
    ...
    self.G_sum = tf.summary.histogram("G", G)

    d_label_smooth = self.d_label_smooth
    d_loss_real = sigmoid_kl_with_logits(D_on_data_logits, 1. - d_label_smooth)
    class_loss_weight = 1.
    d_loss_class = class_loss_weight * tf.nn.sparse_softmax_cross_entropy_with_logits(logits=class_logits,
                                                                                      labels=tf.to_int64(sparse_labels))
