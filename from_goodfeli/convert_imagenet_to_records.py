...
def main(argv):
    pattern = "/home/iindyk/PycharmProjects/my_GAN/ImageNet/n*/*JPEG"
    files = glob(pattern)
    assert len(files) > 0

    dirs = glob("/home/iindyk/PycharmProjects/my_GAN/ImageNet/n*")
    dirs = [d.split('/')[-1] for d in dirs]
    dirs = sorted(dirs)
    str_to_int = dict(zip(dirs, range(len(dirs))))

    outfile = '/home/iindyk/PycharmProjects/my_GAN/ImageNet/imagenet_train_labeled_' + str(IMSIZE) + '.tfrecords'
