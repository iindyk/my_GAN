...
from from_goodfeli.model import save_images
import time
import numpy as np
    ...
    try:
        while not coord.should_stop():
            ...
            if time.time() - print_time > 15.:
                ...
                print("[Batch %(idx)d] time: %(total_time)4.4f, d_loss: %(d_loss).8f, g_loss: %(errG).8f, "
                      "d_loss_real: %(errD_real).8f, d_loss_fake: %(errD_fake).8f, d_loss_class: %(errD_class).8f, "
                      "sec/batch: %(sec_per_batch)4.4f, sec/this batch: %(sec_this_batch)4.4f" \
                      % locals())
